# Org/GitOps guardrails for the rendered eks-chart manifests (+ the ArgoCD
# Application). Kind-agnostic deep-walk. Complements Checkov (per-resource
# misconfig) — this is the DEPLOYED artifact, so image tags must be pinned.
#
# deny -> hard gate; warn -> advisory.
package kubernetes

import rego.v1

approved_image_prefixes := {
	"public.ecr.aws/",
	"registry.k8s.io/",
}

# Ephemeral Helm test hooks (helm.sh/hook: test*) are not the deployment surface
# — they run once under `helm test` and exit — so image/host rules skip them.
is_test_hook if {
	ann := input.metadata.annotations["helm.sh/hook"]
	contains(ann, "test")
}

image_refs contains img if {
	not is_test_hook
	walk(input, [path, value])
	path[count(path) - 1] == "image"
	is_string(value)
	img := value
}

is_ecr(img) if regex.match(`^[0-9]+\.dkr\.ecr\.[a-z0-9-]+\.amazonaws\.com/`, img)

is_approved(img) if is_ecr(img)

is_approved(img) if {
	some prefix in approved_image_prefixes
	startswith(img, prefix)
}

digest_pinned(img) if contains(img, "@sha256:")

image_tag(img) := tag if {
	not digest_pinned(img)
	parts := split(img, "/")
	last := parts[count(parts) - 1]
	contains(last, ":")
	seg := split(last, ":")
	tag := seg[count(seg) - 1]
}

untagged(img) if {
	not digest_pinned(img)
	parts := split(img, "/")
	last := parts[count(parts) - 1]
	not contains(last, ":")
}

# ---------------------------------------------------------------------------
# DENY — this is the live deployment; images must be reproducibly pinned.
# ---------------------------------------------------------------------------

deny contains msg if {
	some img in image_refs
	image_tag(img) == "latest"
	msg := sprintf("image %q uses the mutable :latest tag — the deployed manifest must pin a real tag", [img])
}

deny contains msg if {
	some img in image_refs
	untagged(img)
	msg := sprintf("image %q has no tag or digest — pin an explicit tag", [img])
}

deny contains msg if {
	not is_test_hook
	walk(input, [path, value])
	path[count(path) - 1] == "privileged"
	value == true
	msg := sprintf("%s/%s: a container requests privileged: true", [obj_kind, obj_name])
}

deny contains msg if {
	not is_test_hook
	walk(input, [path, value])
	some key in {"hostNetwork", "hostPID", "hostIPC"}
	path[count(path) - 1] == key
	value == true
	msg := sprintf("%s/%s: %s: true breaks host isolation", [obj_kind, obj_name, key])
}

deny contains msg if {
	not is_test_hook
	walk(input, [path, _])
	path[count(path) - 1] == "hostPath"
	msg := sprintf("%s/%s: mounts a hostPath volume", [obj_kind, obj_name])
}

# ---------------------------------------------------------------------------
# WARN
# ---------------------------------------------------------------------------

warn contains msg if {
	some img in image_refs
	not is_approved(img)
	image_tag(img) != "latest"
	msg := sprintf("%s/%s: image %q is not from the org ECR or the approved allowlist", [obj_kind, obj_name, img])
}

obj_kind := k if {
	k := input.kind
} else := "resource"

obj_name := n if {
	n := input.metadata.name
} else := "?"
